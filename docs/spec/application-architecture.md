...
- Slot resolution is read-only. The Reconciler is the only Selection Ledger write path and runs only on explicit triggers.
...
A single FastAPI application renders the HTML routes from the [HTTP contract](http-contract.md), serves the JSON API, resolves direct images, and exposes one internal reconcile endpoint for the scheduler. The same package provides the CLI and the email automation from the [CLI and email contract](cli-email-contract.md). All three surfaces share domain semantics in-process through the deep modules below; nothing reimplements selection rules.
...
Pure, I/O-free Python. Defines the Daily Slot and its `empty`, `selected`, and `conflict` states, Selection Day and calendar-timezone rules, and a `Clock` protocol so tests control "today". Every date decision in the system flows through this module.

### Slot Catalog

The single read model for Daily Slots. Its interface is the contract's read surface: `get_slot`, `today`, `latest`, `random`, `range`, `archive`, `search`, and `statistics`. Its implementation hides ledger queries, Raindrop content fetches, state derivation, and conflict rules. HTML pages, JSON endpoints, CLI read commands, and email preconditions all consume exactly this module.
...
## Reconciliation And Read Purity

Slot Catalog reads never write. A page view or JSON read is always a read of the Selection Ledger plus Raindrop-authoritative content, so read latency never includes reconciliation work and the ledger stays insert-only from a single writer.
...
Routers are thin: parse and validate input, call a deep module, render or serialize the result, and map domain outcomes to the contract's status codes and error envelope. A middleware assigns each request a `request_id`, included in every error envelope and structured log line. The internal reconcile router authenticates via a configured secret shared with the scheduler. HTML rendering uses Jinja2 templates; progressive-enhancement scripts and stylesheets are static assets.
...
```text
src/daily_miku/
  domain/            # pure core: slots, Selection Day and timezone rules, Clock
  ledger/            # Selection Ledger: port, postgres.py, memory.py fake, migrations/
  raindrop/          # content source: port, client.py adapter, memory.py fake
...
- The interface is the test surface: deep modules are tested through their public interfaces with in-memory fakes for the Ledger, Raindrop, Blob, and SMTP seams.
...
- The `Clock` seam makes Selection Day and future-date behavior deterministic in tests.
...
- **Lazy reconcile-on-read**: couples write timing to traffic, slows reads, and obscures the observation-delay policy.
- **SQLAlchemy/SQLModel with Alembic**: more concepts than two tables justify; numbered SQL files and a version table cover migration and `doctor` needs.
